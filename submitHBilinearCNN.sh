...
python TrainModelHBilinearCNN.py --param-name $1 --num-gpus $num_gpus --num-nodes $num_nodes &
for ((i=1; i<$num_nodes; i++)); do
  node="${nodes[$i]}"
  echo "INFO: Lauching script on node $i"
  ssh "$node" "source ~/.bashrc_$1_$i && python $PWD/TrainModelHBilinearCNN.py --param-name $1 --num-gpus $num_gpus --num-nodes $num_nodes" &
done
