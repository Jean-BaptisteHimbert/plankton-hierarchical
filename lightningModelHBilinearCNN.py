...
class BranchLeaf(nn.Module, ABC):
    def __init__(self, attention_size, num_classes):
        super().__init__()
        self.layer2 = nn.Linear((768 + attention_size)//2, attention_size)
        self.layer3 = nn.Linear(attention_size, int(num_classes))

    def forward(self, x_main):
        return self.layer3(self.layer2(x_main))

class BodyHBCNN(nn.Module, ABC):
    def __init__(self, feature_extractor_path, attention_size, freeze_feature_block1, freeze_feature_block2):
        ...
        self.feature_block3 = copy.deepcopy(model.layers[3].blocks[0])
        
        for param in self.feature_block3.parameters():
            param.requires_grad = True

        self.branch1 = nn.Linear(self.embedding_size, (self.embedding_size + attention_size) //2)
        self.branch2 = nn.Linear(self.embedding_size *2, (self.embedding_size + attention_size) //2)
        self.branch3 = nn.Linear((self.embedding_size*2) + 49, (self.embedding_size + attention_size) //2)
    ...
    def forward(self, x):
        ...
        features3bis = self.feature_block3(features2)
        ...
        f3 = self.flatten(self.avgpool(torch.bmm(features3, torch.transpose(features3bis, 1, 2))))
        
        
        o2 = torch.concat([f1, f2], dim=1)
        o3 = torch.concat([f1, f2, f3], dim=1)
        return self.branch1(f1), self.branch2(o2), self.branch3(o3)

class HB_CNN(nn.Module, ABC):
    def __init__(self,
                 feature_extractor_path: Path,
        ...
        self.branch_body = BodyHBCNN(feature_extractor_path=feature_extractor_path,
    ...
    def forward(self, x):
        ...
        y_1 = self.branch_leaf1(x_1)
        y_2 = self.branch_leaf2(x_2)
        y_3 = self.branch_leaf3(x_3)
...
class HB_CNN_Lightning(LightningModule, ABC):
    def __init__(   self,
                    results: Path,
                    model_name: str,
        ...
        print(f"feature extractor path is: {feature_extractor_path}")
        # create model
        self.model = HB_CNN(feature_extractor_path=feature_extractor_path,
