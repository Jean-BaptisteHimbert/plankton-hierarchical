def main(training_parameters_name, num_gpus, num_nodes):
    ...
    freeze_binary_head = parameters["freeze_binary_head"]
    training_only_binary_head = parameters["training_only_binary_head"]
    ...
    best_models_checkpoint_callback0 = ModelCheckpoint(
        dirpath=BEST_MODEL_DIR, save_top_k=1, verbose=False, monitor="val_binary_f1", mode="max"
    )
    best_models_checkpoint_callback1 = ModelCheckpoint(
        dirpath=BEST_MODEL_DIR, save_top_k=1, verbose=False, monitor="val_2_1_f1", mode="max"
    )
    best_models_checkpoint_callback2 = ModelCheckpoint(
        dirpath=BEST_MODEL_DIR, save_top_k=1, verbose=False, monitor="val_2_2_f1", mode="max"
    )
    ...
    early_stop_callback0 = EarlyStopping(monitor="val_binary_f1", min_delta=0.0001, patience=30, verbose=True, mode="max")
    early_stop_callback1 = EarlyStopping(monitor="val_2_1_f1", min_delta=0.0001, patience=30, verbose=True, mode="max")
    early_stop_callback2 = EarlyStopping(monitor="val_2_2_f1", min_delta=0.0001, patience=30, verbose=True, mode="max")
    early_stop_callback3 = EarlyStopping(monitor="val_2_2_f1", min_delta=0.0001, patience=30, verbose=True, mode="max")


    swa_callback = StochasticWeightAveraging(swa_lrs=1e-3)

    checkpoint0 = ModelCheckpoint(monitor="val_binary_f1", mode="min")
    checkpoint1 = ModelCheckpoint(monitor="val_2_1_f1", mode="min")
    checkpoint2 = ModelCheckpoint(monitor="val_2_2_f1", mode="min")
    checkpoint3 = ModelCheckpoint(monitor="val_2_2_f1", mode="min")
    ...
    modelTest = SeggregatedBACNN.load_from_checkpoint(ckpt_path)
...
if __name__ == "__main__":
    ...
    from libraries.lightningDM import ImageFolderLightningDataModule
