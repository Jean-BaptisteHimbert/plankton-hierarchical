...
class HierarchicalFolder(ImageFolder):
    def __init__(self,root_dir):
        ...
        self.image_paths = []
        self.labels = []
        num_classes = np.zeros((2, 2))

        # Iterate over the image files and their directories
        first_level_classes = sorted([i for i in os.listdir(self.root_dir) if not i.startswith('.')])
        for i, first_level_class in enumerate(first_level_classes):
            second_level_classes = sorted([i for i in os.listdir(os.path.join(self.root_dir, first_level_class)) if not i.startswith('.')])
            max_1 = len(second_level_classes)
            max_2 = 0
            for j, second_level_class in enumerate(second_level_classes):
                third_level_classes = sorted([i for i in os.listdir(os.path.join(self.root_dir, first_level_class, second_level_class)) if not i.startswith('.')])
                for k, third_level_class in enumerate(third_level_classes):
                    ...
                    for img_name in images_list:
                        if img_name.endswith('.jpeg') or img_name.endswith('.jpg') or img_name.endswith('.png'):
                            ...
                            if i == 0:
                                self.labels.append([i, j, k+max_2, 0, 0])
                            else:
                                self.labels.append([i, 0, 0, j, k+max_2])
                max_2 += len(third_level_classes)
            num_classes[i, 0] = max_1
            num_classes[i, 1] = max_2
        ...
        for i, first_level_class in enumerate(first_level_classes):
            ...
            for j, second_level_class in enumerate(second_level_classes):
                ...
                for k, third_level_class in enumerate(third_level_classes):
                    ...
                    if i == 0:
                        target = [i, j, k+max_2, 0, 0]
                    else:
                        target = [i, 0, 0, j, k+max_2]
                    int_target = i + (10*j) + (100*k)
